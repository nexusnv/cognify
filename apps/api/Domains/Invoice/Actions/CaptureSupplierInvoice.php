...
class CaptureSupplierInvoice
{
    ...
    public function handle(PurchaseOrder $purchaseOrder, User $actor, array $payload): SupplierInvoice
    {
        try {
            ...
            if (! $this->isDuplicateInvoiceViolation($exception)) {
                throw $exception;
            }
            ...
            $matchingInvoice = SupplierInvoice::query()
                ...
                ->where('invoice_number_normalized', SupplierInvoiceNumber::normalize((string) $payload['invoiceNumber']))
    ...
    private function isDuplicateInvoiceViolation(QueryException $exception): bool
    {
        $message = strtolower($exception->getMessage());

        return str_contains($message, 'supplier_invoices_tenant_po_number_unique')
            || (str_contains($message, 'unique') && str_contains($message, 'supplier_invoices'));
    }
}
