...
import { Alert, AlertDescription, Badge, Button, Card, CardContent, CardHeader, CardTitle, Input, Textarea } from "@cognify/ui";
...
export function RfqAwardPoHandoffPanel({ rfqId, handoff, isLoading, error }: RfqAwardPoHandoffPanelProps) {
  ...
  return (
    <Card id="po-handoff" role="region" aria-label="PO request handoff">
      <CardHeader>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <CardTitle className="text-base">PO request handoff</CardTitle>
      ...
      {error ? (
        <Alert variant="destructive" className="mt-3"><AlertDescription>{getMutationErrorMessage(error)}</AlertDescription></Alert>
      ) : null}
      ...
      {actionError ? (
        <Alert variant="destructive" className="mt-3"><AlertDescription>{getMutationErrorMessage(actionError)}</AlertDescription></Alert>
      ) : null}
      </CardContent>
    </Card>
  );
