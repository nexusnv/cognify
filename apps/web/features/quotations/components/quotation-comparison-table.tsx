      ...
      <div className="overflow-hidden">
        <Table className="min-w-full text-sm">
          <TableHeader className="bg-muted/40 text-left">
            <TableRow>
              <TableHead scope="col" className="min-w-56 px-4 py-3 font-medium">RFQ line</TableHead>
              {vendors.map((vendor) => (
                <TableHead key={vendor.vendorId} scope="col" className="min-w-64 px-4 py-3 font-medium">
                  {vendor.vendorName}
                </TableHead>
              ))}
            </TableRow>
          ...
          <TableBody>
            {rows.map((row) => (
              <TableRow key={row.rfqLineItemId} className="align-top">
                <TableHead scope="row" className="px-4 py-4 text-left font-medium">
