      ...
      <div className="overflow-hidden">
        <Table className="min-w-full text-sm">
          <TableHeader className="bg-muted/40 text-left">
            <TableRow>
              <TableHead scope="col" className="min-w-48 px-4 py-3 font-medium">Term</TableHead>
              {vendors.map((vendor) => (
                <TableHead key={vendor.vendorId} scope="col" className="min-w-48 px-4 py-3 font-medium">
                  {vendor.vendorName}
                </TableHead>
              ))}
            </TableRow>
          ...
          <TableBody>
            {terms.map((term) => (
              <TableRow key={term.id}>
                <TableHead scope="row" className="px-4 py-3 text-left font-medium">{term.label}</TableHead>
                {vendors.map((vendor) => {
