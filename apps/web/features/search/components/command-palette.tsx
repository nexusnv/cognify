...
export function CommandPalette({
  ...
  return (
    <CommandDialog
      open={open}
      title="Command menu"
      description="Search commands, recent records, and requisitions."
      className="top-24 translate-y-0 gap-0 p-0 sm:max-w-[48rem]"
    ...
    >
      ...
      <CommandList className="max-h-[min(70vh,34rem)] p-2">
        {searchState.errorMessage ? (
          <div
            role="alert"
            className="mx-1 mb-2 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-900"
          >
            {searchState.errorMessage}
          </div>
        ) : null}
        ...
        {visibleCommands.length > 0 ? (
          <CommandGroup heading="Commands" className="pb-2">
        ...
        {visibleRecentRecords.length > 0 && !showRemoteSearch ? (
          <CommandGroup heading="Recent records" className="pb-2">
